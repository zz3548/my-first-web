...
이번 단계에서는 페이지별 로딩/빈 상태/오류 UX와 서버/네트워크 에러를 사용자 친화적으로 처리하는 규칙을 코드 기반으로 정리합니다.

- 추가된 파일
  - `app/posts/loading.tsx` — 포스트 목록용 로딩 스켈레톤
  - `app/posts/error.tsx` — 라우트 수준 오류 안내(재시도/목록 돌아가기 버튼 포함)
  - `lib/error-message.ts` — 서버(예: Supabase) 및 네트워크 오류의 원문을 친절한 메시지로 변환하는 유틸

- 화면별 상태 설계
  - `/posts` (목록)
    - 로딩: 스켈레톤(펄스)으로 시각적 피드백 제공
    - 빈 상태: 친절한 안내 문구(첫 글 작성 권장)
    - 오류: 라우트 에러 또는 목록 영역 내 경고로 사용자 안내
  - `/posts/[id]` (상세)
    - 로딩: 상세 내 기존 로딩 UI 사용
    - not-found: `notFound()`로 처리하거나 친절한 안내 후 목록으로 유도
    - 오류: 화면에 친절한 문구 표시, 개발자 콘솔에 원문 로그 남김
  - `/posts/new` (작성)
    - 클라이언트 유효성 검증: 제목(>=2자), 내용(>=10자)
    - 필드별 에러: 입력 하단에 빨간색 텍스트로 표시
    - 제출 중 UX: 제출 버튼 비활성화 및 상태 텍스트(중복 제출 방지)
  - 인증 페이지(`/login`, `/signup`)
    - Supabase/네트워크 에러는 `lib/error-message.ts`로 변환하여 사용자에게 노출
    - 개발자용 원문은 `console.error`에 기록

- 에러 메시지 변환 규칙 (`lib/error-message.ts`)
  - 권한/RLS 관련(예: Postgres `42501`, "row-level") → "이 작업을 수행할 권한이 없습니다."
  - 네트워크 오류(예: "Failed to fetch") → "인터넷 연결을 확인해주세요."
  - not found 계열(예: `404`, "not found", "no rows") → "요청한 게시글을 찾을 수 없습니다."
  - 그 외: "일시적인 오류가 발생했습니다. 잠시 후 다시 시도해주세요."

- 설계 원칙
  - 사용자 경험 우선: 기술적 원문 대신 친절한 안내 제공
  - 개발자 디버깅 지원: 콘솔 로그에 원문을 남겨 문제 추적 가능
  - 라이브러리 최소화: 새로운 패키지 추가 없이 Tailwind + 기존 컴포넌트로 구현

해당 변경은 문서화된 규칙을 따라 일관된 UX를 보장하며, 서비스 운영 중 발생하는 네트워크·권한 문제에 대해 사용자와 개발자 각각에 적합한 피드백을 제공합니다.
