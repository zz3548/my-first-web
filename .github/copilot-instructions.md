...
- 보안 결정은 클라이언트의 `if` 분기(예: `if (user.id === post.user_id)`)에 의존하지 마세요 — UI 분기는 UX 용도로만 사용하고, 데이터 접근 제어는 RLS로 강제하세요.
- RLS SQL은 반드시 마이그레이션 파일로 남기고(`supabase/migrations/`), 배포 파이프라인에서 적용하세요. 콘솔 실험만으로 운영을 대신하지 마십시오.
- `service_role` 키는 클라이언트에 절대 포함하지 마세요. 서비스 역할 키는 서버 전용 환경변수로만 사용하고, 필요한 서버 작업만 해당 키로 수행하세요.
